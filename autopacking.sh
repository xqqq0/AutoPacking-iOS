...
# 判读用户是否有输入
if test[ "$__SCHEME_NAME_SELECTED" == "1" ]; then
...
# 判读用户是否有输入
if test[ "$__BUILD_CONFIGURATION_SELECTED" == "1" ]; then
...
# 判读用户是否有输入
if test[ "$__IS_WORKSPACE_SELECTE" == "1" ]; then
	__IS_WORKSPACE=true
...
# 判读用户是否有输入
if test[ "$__IS_AUTO_OPENT_FILE_SELECTED" == "1" ]; then
...
# 判读用户是否有输入
if test[ "$__IS_START_PACKINF" == "1" ]; then
...
if $__IS_WORKSPACE ; then
	...
	if test[ $__BUILD_CONFIGURATION == "Debug" ]; then
	...
	if test[ $__BUILD_CONFIGURATION == "Debug" ] ; then
		echo "$__LINE_BREAK_LEFT您选择了以 xcodeproj-Debug 模式打包 $__LINE_BREAK_RIGHT"
		# step 1. Clean
...
if test -f "$__EXPORT_IPA_PATH/$__IPA_NAME.ipa" ; then
	...
	then
		...
		if test[ "$__UPLOAD_TYPE_SELECTED" == "1" ] ; then
